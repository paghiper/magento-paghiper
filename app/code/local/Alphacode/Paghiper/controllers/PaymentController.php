...
class Alphacode_Paghiper_PaymentController extends Mage_Core_Controller_Front_Action
{
    public function gatewayAction()
    {
        if ($this->getRequest()->get('orderId')) {
            ...
            Mage_Core_Controller_Varien_Action::_redirect('paghiper/payment/response', ['_secure' => false, '_query' => $arr_querystring]);
        }
    }
    ...
    public function redirectAction()
    {
        ...
        $this->loadLayout();
        $block = $this->getLayout()->createBlock('Mage_Core_Block_Template', 'paghiper', ['template' => 'paghiper/redirect.phtml', 'response' => $response]);
    ...
    public function responseAction()
    {
        if ($this->getRequest()->get('flag') == '1' && $this->getRequest()->get('orderId')) {
            ...
            Mage_Core_Controller_Varien_Action::_redirect('checkout/onepage/error', ['_secure' => false]);
        }
    }
    ...
    public function generateBankSlip()
    {
        ...
        foreach ($orderItems as $sItem) {
            $price = $sItem->getproduct()->isBundle() && $sItem->getproduct()->getPriceType() === '0' // bundle with dynamic price
                ? 0 // the price will be get through it's children
                : $sItem->getPrice();
            $products['produto_codigo_' . $x] = $sItem->getId();
            $products['produto_valor_' . $x] = $price;
    ...
    {
        $protocols = [0 => 'https://www.', 1 => 'https://', 2 => 'http://www.', 3 => 'http://', 4 => 'www.', 5 => ''];
